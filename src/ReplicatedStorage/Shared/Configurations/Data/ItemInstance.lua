...
function ItemInstance.Create(ItemId: string, Quantity: number?): ItemInstance?
	...
	if ItemDef.BaseStats then
		Instance.Info = {}
		for StatName, StatValue in ItemDef.BaseStats do
...
function ItemInstance.GetStat(Instance: ItemInstance, StatName: string): number?
	...
	if not ItemDef or not ItemDef.BaseStats then
		return nil
	end

	return ItemDef.BaseStats[StatName]
end
...
function ItemInstance.RepairDurability(Instance: ItemInstance, Amount: number)
	...
	local ItemDef = ItemDatabase.GetItem(Instance.Id)
	if not ItemDef or not ItemDef.BaseStats or not ItemDef.BaseStats.Durability then
		return
	end

	local MaxDurability = ItemDef.BaseStats.Durability
...
function ItemInstance.GetMaxDurability(Instance: ItemInstance): number?
	...
	return ItemDef.BaseStats.Durability
end
